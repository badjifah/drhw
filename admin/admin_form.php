...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nom) || empty($login) || empty($email)) {
        flash('danger', 'Nom, login et email sont obligatoires.');
        redirect(BASE_URL . "/admin/admin_form.php" . ($id ? "?id=$id" : ''));
    }

    // Check unique login
    $exists = $db->fetch("SELECT id FROM admins WHERE login = :login AND id != :id", ['login' => $login, 'id' => $id ?? 0]);
    if ($exists) {
        flash('danger', 'Ce login est déjà utilisé.');
        redirect(BASE_URL . "/admin/admin_form.php" . ($id ? "?id=$id" : ''));
    }

    $data = ['nom' => $nom, 'login' => $login, 'email' => $email, 'role' => $role];
    ...
    if ($isEdit) {
        ...
        logActivity('admin_updated', "Admin: $nom");
        ...
        logActivity('admin_created', "Admin: $nom");
        ...
        <div class="card-body">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="nom" value="<?= sanitize($admin['nom'] ?? '') ?>" class="form-input" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Login *</label>
                    <input type="text" name="login" value="<?= sanitize($admin['login'] ?? '') ?>" class="form-input" required>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
